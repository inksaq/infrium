  ...
  protected void onItemClick(Player player, ItemStack clickedItem, Inventory inventory) {
    if (clickedItem.getType().name().contains("WOOL") && clickedItem.getType() != Material.RED_WOOL) {
      ...
      if ((Objects.equals(serverName, com.infrium.core.ICore.getInstance().getServerName()))) {
        com.infrium.core.ICore.getInstance().getInfriumProvider().craftInfriumPlayer(player).sendTitle(
  ...
  public ItemStack craftItem(Server server) {
    var mat =
            (server.getServerStatus().getOnlinePlayers() >= server.getServerType().getMaxPlayers() / 2
                    ...
                    :  (Objects.equals(server.getName(), com.infrium.core.ICore.getInstance().getServerName()) ? Material.GREEN_WOOL : Material.WHITE_WOOL));
